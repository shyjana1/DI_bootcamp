...
right.addEventListener('click', getData(`rando`))
left.addEventListener('click', getData(`${randm}`))
// let fetchApi = (async function () {
    ...
    // }
        ...
        async function getData() {
            try {
                let rando = Math.round(Math.random() * 81)
                let random = Math.round(Math.random() * 81)
